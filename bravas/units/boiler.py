...
# Creating the rest of streams
makeup_water = bst.Stream('boiler_makeup_water')
air = bst.Stream('combustion_air')

emissions = bst.Stream('boiler_emissions')
steam = bst.Stream('produced_stream')
ash = bst.Stream('ash_disposal')


# Create the unit operation for the biomass boiler
boiler = BiomassBoiler(
    ID = 'B-101',
    ins = (biomass, makeup_water, air),
    outs = (emissions, steam, ash),
    boiler_efficiency = 0.80,
    LHV = LHV,
...
# Create the system
system = bst.System('biomass_boiler_system', path = (boiler,))
system.simulate()

# Mass balance
biomass_mass_flow = biomass.F_mass
print('Biomass input [kg/hr]:', biomass_mass_flow)
makeup_water_mass_flow = makeup_water.F_mass
print('Makeup water input [kg/hr]:', makeup_water_mass_flow)
air_mass_flow = air.F_mass
print('Air input [kg/hr]:', air_mass_flow)

steam_mass_flow = steam.F_mass
print('Steam output [kg/hr]:', steam_mass_flow)
emissions_mass_flow = emissions.F_mass
print('Emissions output [kg/hr]:', emissions_mass_flow)
ash_mass_flow = ash.F_mass
print('Ash output [kg/hr]:', ash_mass_flow)

input = biomass.F_mass + makeup_water.F_mass + air.F_mass
output = emissions.F_mass + steam.F_mass + ash.F_mass

balance = input - output # This should be 0
print('Mass balance difference [kg/hr]:', balance)

# Energy balance
hu = boiler.heat_utilities[0]
print('Heat duty [kJ/hr]:', hu.duty) 